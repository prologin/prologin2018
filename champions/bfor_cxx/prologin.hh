...
/// Renvoie le plus court chemin entre deux positions de la banquise sous la forme d'une suite de direction à emprunter. Si la position est invalide ou que le chemin n'existe pas, le chemin renvoyé est vide.
...
/// Indique la position de l'agent sur la banquise désigné par le numéro ``id_agent`` appartenant au joueur ``id_joueur``.
...
inline bool operator==(const alien_info& a, const alien_info& b) {
  ...
  if (a.points_capture != b.points_capture) return false;
  if (a.tour_invasion != b.tour_invasion) return false;
  if (a.duree_invasion != b.duree_invasion) return false;
  if (a.capture_en_cours != b.capture_en_cours) return false;
  return true;
}

inline bool operator!=(const alien_info& a, const alien_info& b) {
  if (a.pos != b.pos) return true;
  if (a.points_capture != b.points_capture) return true;
  if (a.tour_invasion != b.tour_invasion) return true;
  if (a.duree_invasion != b.duree_invasion) return true;
  if (a.capture_en_cours != b.capture_en_cours) return true;
  return false;
}
...
namespace std {
  template <>
  struct hash<alien_info>
  {
    std::size_t operator()(const alien_info& s)
    {
      std::size_t res = 0;
      res ^= 0x9e3779b9 + (res << 6) + (res >> 2) + std::hash<position>()(s.pos);
      res ^= 0x9e3779b9 + (res << 6) + (res >> 2) + std::hash<int>()(s.points_capture);
      res ^= 0x9e3779b9 + (res << 6) + (res >> 2) + std::hash<int>()(s.tour_invasion);
      res ^= 0x9e3779b9 + (res << 6) + (res >> 2) + std::hash<int>()(s.duree_invasion);
      res ^= 0x9e3779b9 + (res << 6) + (res >> 2) + std::hash<int>()(s.capture_en_cours);
      return res;
    }
  };
}
...
inline bool operator==(const action_hist& a, const action_hist& b) {
  ...
  if (a.id_agent != b.id_agent) return false;
...
inline bool operator!=(const action_hist& a, const action_hist& b) {
  ...
  if (a.id_agent != b.id_agent) return true;
...
namespace std {
  ...
  struct hash<action_hist>
  {
    ...
    {
      std::size_t res = 0;
      res ^= 0x9e3779b9 + (res << 6) + (res >> 2) + std::hash<action_type>()(s.type);
      res ^= 0x9e3779b9 + (res << 6) + (res >> 2) + std::hash<int>()(s.id_agent);
